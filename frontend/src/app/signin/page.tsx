...
import { Mail, Phone, Lock, Eye, EyeOff, Sun, Moon, ArrowRight, AlertCircle } from "lucide-react";
...
export default function SignIn() {
  ...
  // Form states
  const [identifier, setIdentifier] = useState(""); // can be email or 10-digit mobile
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || "Authentication failed. Incorrect credentials.");
      }
    ...
    <div className="min-h-screen relative overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex items-center justify-center p-6 transition-colors duration-300">
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-slate-400 font-medium">Welcome back! Log in to build your resume.</p>
        </div>
        ...
        <div className="p-8 rounded-2xl border border-slate-200 dark:border-slate-900 bg-white/70 dark:bg-slate-900/40 backdrop-blur-md shadow-2xl">
          <form onSubmit={handleSignIn} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Email or Mobile Number</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl text-sm font-bold bg-gradient-to-r from-emerald-500 to-indigo-500 text-white shadow-xl hover:shadow-emerald-500/10 hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2"
            >
